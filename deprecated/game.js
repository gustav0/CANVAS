...
var M=function(){
    ...
    for(var i=0;i<L.length;i++){
        if(L[i][0]>0){
            var d=new Date();
            d.setTime(L[i][1]);
            var a=d.getDate(),month=d.getMonth()+1,year=d.getFullYear();
            document.getElementById("score_"+i).innerHTML=L[i][0]+' ('+month+'/'+a+'/'+year+')'
            }
        }
    };
...
var N=function(){
    if(typeof(Storage)!=="undefined"){
        var a=[];
        for(var i=0;i<L.length;i++)a.push(L[i].join(','));
...
var Q=function(a){
    ...
    if(-1 in H){
        ...
        if(A.yvel<(b*accel_time))A.yvel+=b*a
            ...
            A.current+=a;
    if(A.current>2)A.current=0;else A.current+=a;
    J.x-=J.speed*a;
    ...
    if(D.x-D.speed*a<=-m.width)D.x=m.width;else D.x-=D.speed*a;
    if(E.x-E.speed*a<=-m.width)E.x=m.width;else E.x-=E.speed*a;
    if(J.x<=-33)J.reset();
...
var R=function(){
    ...
    for(var i=P.length-1;i>=0;i--){
        n.drawImage(invincible?((P[i][2]==0)?x:y):((P[i][2]==0)?v:w),A.x-100*(distance-P[i][0]),P[i][1]+20)
        }
        n.strokeStyle='#ff0';
    n.lineWidth=1;
    n.stroke();
    ...
    if(typeof(global_ceiling)!='undefined'){
        ...
        if(dead)n.drawImage(r,A.x,A.y);else n.drawImage(invincible?s:q,(1-Math.floor(A.current*4)%2)*A.w,(-1 in H)?A.h:0,A.w,A.h,A.x,A.y,A.w,A.h);
    if(dead){
        n.fillStyle="rgb(250, 250, 250)";
        n.font="24px Helvetica";
        n.textAlign="center";
        n.textBaseline="top";
        n.fillText("Game Over",(m.width+A.w-45)/2,A.y>m.height/2?A.y-30:A.y+50)
        }
        n.fillStyle="rgb(250, 250, 250)";
    n.font="24px Helvetica";
    n.textAlign="left";
...
if(!localStorage.highscores)localStorage.highscores='0,0;0,0;0,0';
M()
}/* 
